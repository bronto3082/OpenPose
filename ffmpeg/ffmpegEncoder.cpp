...
bool ffmpegEncoder::InitFile(std::string& inputFile, std::string& container)
{
	bool res = false;
...
bool ffmpegEncoder::AddFrame(AVFrame* frame, const char* soundBuffer, int soundBufferSize)
{
	bool res = true;
	int nOutputSize = 0;
...
AVFrame* ffmpegEncoder::CreateFFmpegPicture(AVPixelFormat pix_fmt, int nWidth, int nHeight)
{
...
bool ffmpegEncoder::OpenVideo(AVFormatContext* oc, AVStream* pStream)
{
	AVCodec* pCodec;
...
void ffmpegEncoder::CloseVideo(AVFormatContext* pContext, AVStream* pStream)
{
...
bool ffmpegEncoder::NeedConvert()
{
	bool res = false;
...
AVStream* ffmpegEncoder::AddVideoStream(AVFormatContext* pContext, AVCodecID codec_id)
{
...
AVStream* ffmpegEncoder::AddAudioStream(AVFormatContext* pContext, AVCodecID codec_id)
{
	AVCodecContext* pCodecCxt = NULL;
...
bool ffmpegEncoder::OpenAudio(AVFormatContext* pContext, AVStream* pStream)
{
	AVCodecContext* pCodecCxt = NULL;
	AVCodec* pCodec = NULL;
	pCodecCxt = pStream->codec;
...
void ffmpegEncoder::CloseAudio(AVFormatContext* pContext, AVStream* pStream)
{
	avcodec_close(pStream->codec);
...
bool ffmpegEncoder::AddVideoFrame(AVFrame* pOutputFrame, AVCodecContext* pVideoCodec)
{
	bool res = false;
...
bool ffmpegEncoder::AddAudioSample(AVFormatContext* pFormatContext, AVStream* pStream,
